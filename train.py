...
def main(args):
    ...
    train_data = BUTIDDataset(path=train_label_paths[args.dataset], 
                              args=args,
                              phase='train')
    print(train_data)
    print(train_data[10])
...
def evaluate(args, data_loader, model, model_without_ddp, phase):
    ...
    if args.dataset == 'CSL_Daily':
        tgt_pres = [' '.join(list(r.replace(" ",'').replace("\n",''))) if task == "SLT" else r for r, task in zip(tgt_pres, tasks)]
        tgt_refs = [' '.join(list(r.replace("，", ',').replace("？","?").replace(" ",''))) if task == "SLT" else r for r, task in zip(tgt_refs, tasks)]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--modes', nargs='+', default=['body', 'right', 'left'], type=str, help='input modes: body, right, left, face'
    )
    parser.add_argument(
