...
@dataclass
class UniSignConfig:
    modes: list = field(default_factory=lambda: ['body', 'left', 'right'])
    hidden_dim: int = 256
...
class UniSign(nn.Module):
    def __init__(self, config):
        ...
        for index, mode in enumerate(self.modes):
            self.gcn_modules[mode], final_dim = get_stgcn_chain(config.proj_dim, 'spatial', (1, spatial_kernel_size), A[index].clone(), True)
    ...
    def forward(self, src_input, tgt_input):
        ...
        for part in self.modes:
            ...
            if part == 'body':
                ...
                elif part == 'face': gcn_feat = gcn_feat + body_feat[..., 0][...,None].detach()
        ...
        prefix_token = self.mt5_tokenizer(
            ["Translate sign language video into {lang} spoken language text: ".format(lang=self.tgt_lang)] * inputs_embeds.size(0),
            padding="longest",
            truncation=True,
            return_tensors="pt",
        ...
        tgt_input_tokenizer = self.mt5_tokenizer(tgt_input['labels'], 
                                                return_tensors="pt", 
                                                padding=True,
                                                truncation=True,
                                                max_length=50)
        ...
        loss_fct = torch.nn.CrossEntropyLoss(label_smoothing=self.config.label_smoothing, ignore_index=-100)
